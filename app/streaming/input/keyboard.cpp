...
#ifdef Q_OS_DARWIN
    const bool pasteModifier = (event->keysym.mod & KMOD_GUI) != 0;
#else
    const bool pasteModifier = (event->keysym.mod & KMOD_CTRL) != 0;
#endif
    if (m_DirectClipboardPaste && pasteModifier &&
            !(event->keysym.mod & (KMOD_ALT | KMOD_SHIFT)) &&
            (event->keysym.sym == SDLK_v || event->keysym.scancode == SDL_SCANCODE_V)) {
        if (event->state == SDL_PRESSED) {
            performSpecialKeyCombo(KeyComboPasteText);
        }

        // Consume both edges. performSpecialKeyCombo() releases the modifier on
        // key-down, so forwarding the V key-up would create an unmatched event.
        return;
    }
